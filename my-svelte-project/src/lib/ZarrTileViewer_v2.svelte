<script>
  ...
  import ZarrTileSource from './ZarrTileSource_old';
  ...
  function updateMinMax() {
    if (zarrSource) {
        zarrSource.setMinMax(minValues, maxValues);
    }
  }
...
<div>
  ...
  <label>
    ...
    {#each cIndices as index, i}
      <div>
        <label>Channel {index} Min: <input type="number" bind:value={minValues[i]} oninput={() => updateMinMax()}></label>
        <label>Max: <input type="number" bind:value={maxValues[i]} oninput={() => updateMinMax()}></label>
      </div>
    {/each}
  </label>
