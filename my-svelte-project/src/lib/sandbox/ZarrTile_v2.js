...
/**
 * Normalize image data using a given min/max range.
 * @param {Uint8ClampedArray} imageData - The raw grayscale image data.
 * @param {number} minVal - Minimum value for normalization.
 * @param {number} maxVal - Maximum value for normalization.
 * @returns {Uint8ClampedArray} - Normalized image.
 */
function normalizeImage(imageData, minVal, maxVal) {
...
class ZarrTile extends ImageTile {
    constructor(tileCoord, state, source, node, tIndex, cIndices, zIndex) {
        ...
        this.maxImagesInCache = 100;
        // this.rawGrayscaleImages = []; // Cache raw grayscale images
        this.minPixelValues = new Array(cIndices.length).fill(0);
        this.maxPixelValues = new Array(cIndices.length).fill(255);
    ...
    async loadTile() {
        try {
            console.log('image cache', this.imageCache.keys());
            ...
            for (let i = 0; i < this.source.cIndices.length; i++) {  
                ...
                const key = tileArrayPath + '_' + tileSlice.toString();
                console.log('attempting get for', key);

                if (this.imageCache.has(key)) {
                    console.log(key, 'found in cache');
                    rawGrayscaleImages.push(this.imageCache.get(key));
                } else {
                    console.log(key, 'not found in cache, getting tile slice', tileSlice);
                    ...
                    rawGrayscaleImages.push(tileData);
                    this.addToImageCache(key, tileData);
                }
            ...
            for (let i = 0; i < this.source.cIndices.length; i++) { 
                const adjustedImage = normalizeImage(rawGrayscaleImages[i], this.minPixelValues[i], this.maxPixelValues[i]);
                adjustedImages.push(adjustedImage)
            }
    
            const rgbaData = applyPseudocolor(this.colors.slice(0, adjustedImages.length), adjustedImages, this.image.width, this.image.height);
    ...
    setMinMax(minValues, maxValues) {
        ...
        this.changed(); // Refresh the tile layer with the new min/max
    }
    ...
    addToImageCache(key, image) {
        this.imageCacheOrder.unshift(key);
        this.imageCache.set(key, image);

        if (this.imageCache.size > this.maxImagesInCache) {
            const k = this.imageCacheOrder.pop();
